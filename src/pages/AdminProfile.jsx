...
const AdminProfile = () => {
  ...
  const handleSaveEnvironment = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000'}/api/environments`, {
        method: 'POST',
      ...
      if (response.ok) {
        ...
        setShowFormBuilder(false); // Collapse form area on success entry point
      } else {
        throw new Error("Database reject instance exception.");
      }
    } catch (error) {
    ...
    <div className="flex-1 max-w-[900px] mx-auto p-4 sm:p-8 font-epilogue transition-colors">
      ...
      <div className="w-full bg-white dark:bg-[#1c1c24] border border-slate-200 dark:border-[#3a3a43] rounded-3xl p-6 sm:p-8 shadow-xl transition-colors mb-8 flex flex-col sm:flex-row items-center gap-6">
        <div className="w-24 h-24 rounded-2xl bg-gradient-to-tr from-[#8c6dfd] to-[#7a59e6] flex items-center justify-center text-4xl text-white shadow-md shadow-[#8c6dfd]/20 font-bold select-none">
          {userStatus?.name ? userStatus.name.substring(0, 2).toUpperCase() : "AD"}
        </div>
        <div className="flex-1 text-center sm:text-left">
          <div className="flex flex-col sm:flex-row sm:items-center gap-2">
            <h1 className="text-2xl font-extrabold text-slate-900 dark:text-white">{userStatus?.name || "Administrator Hub"}</h1>
          ...
          <p className="text-sm text-slate-500 dark:text-gray-400 mt-1">{userStatus?.email || "admin-system@ecosystem.org"}</p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        ...
        <button 
          ...
          className="p-5 bg-[#8c6dfd] hover:bg-[#7a59e6] text-white rounded-2xl shadow-lg transition-all flex flex-col justify-center items-start group"
        >
              ...
              <form onSubmit={handleSaveEnvironment} className="space-y-6">
                <div className="grid grid-cols-1 gap-4">
                  <div className="flex flex-col gap-2">
                    ...
                    <input 
                      type="text"
                      placeholder="e.g., Computer Science Final Projects"
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <button 
                      ...
                      className="text-xs font-bold text-[#8c6dfd] hover:underline px-3 py-1 bg-[#8c6dfd]/5 rounded-lg"
                    >
                      + Add Question Field
                    </button>
                  </div>
                  ...
                  {questions.map((q, idx) => (
                    ...
                    >
                      ...
                      <button
                        ...
                        className="p-2 bg-red-500/10 hover:bg-red-500/20 text-red-500 rounded-lg disabled:opacity-30 disabled:pointer-events-none mb-0.5 h-9 w-9 flex items-center justify-center text-xs"
                      >
                        🗑️
                      </button>
                    </motion.div>
                ...
                <div className="pt-4">
                  <button
                    type="submit"
                    disabled={isSaving}
                    className="w-full py-3 bg-[#8c6dfd] hover:bg-[#7a59e6] text-white font-bold rounded-xl text-sm transition-colors disabled:opacity-50 shadow-md shadow-[#8c6dfd]/20"
                  >
