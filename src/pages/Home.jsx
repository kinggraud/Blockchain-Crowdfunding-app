...
const Home = () => {
  ...
  // 🔑 Attach fromDashboard: true to filtered campaigns for DisplayCampaigns -> FundCard navigation
  const dashboardCampaigns = filteredCampaigns.map((campaign) => ({
    ...campaign,
    fromDashboard: true,
  }));
  ...
  return (
    <div className="w-full space-y-12 pb-12 font-epilogue text-slate-900 dark:text-white transition-colors duration-300">
      ...
      {/* 🏛️ 0. ACTIVE ADMIN FUNDING ENVIRONMENTS */}
      <div className="pt-4">
        ...
        {environments.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {environments.map((env) => (
              <div 
                key={env.id || env._id || env.environmentId} 
                ...
                <button
                  onClick={() => {
                    ...
                    navigate(`/create-campaign?envId=${selectedEnvId}`, { 
                      state: { environment: env, fromDashboard: true } 
                    });
                  }}
            ...
            {featuredCampaigns.map((campaign, index) => (
              <div 
                ...
                onClick={() => navigate(`/campaign-details/${campaign.pId}`, { 
                  state: { ...campaign, fromDashboard: true } 
                })}
      ...
      <div ref={campaignRef} className="space-y-6">
        ...
        {/* Handing over campaigns with embedded fromDashboard state flag */}
